...
export default defineEventHandler(async (event) => { 
  ...
  let filePath = process.env.MOCK_PUCKERING_PATH;
  ...
  const r = sortedData.map((item, index) => {
    ...
    return {
      ...
      values: [
        item.nucleotide !== "-" ? item.north : null,
        item.nucleotide !== "-" ? item.east : null,
				item.nucleotide !== "-" ? item.west : null,
        item.nucleotide !== "-" ? item.south : null
      ]
    };
  });
